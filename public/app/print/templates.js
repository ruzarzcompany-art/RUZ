/**
 * قوالب النماذج القابلة للطباعة. كل قالب يستقبل حزمة البيانات
...
import { formatDate, formatDateTime, formatMoney, label } from "../api.js?v=v8";
 ...
 *
 * البنود المسجَّلة في النظام (آخر مسير راتب، المكافآت المعتمدة، رصيد السلف،
 * قيمة العهد غير المُعادة) تُطبع بأرقامها ويُجمع صافيها. وما لا يحسبه النظام
 * (مكافأة نهاية الخدمة، بدل الإجازة غير المستحقة) يُطبع خانات فارغة تُعبّأ
 * يدوياً — الصافي المطبوع يُوصف بأنه «من البنود المسجَّلة» كي لا يُقرأ الرقم
 * على أنه يشمل بنوداً لم تُحسب.
 *
 * جهة طلب إنهاء العقد تصل في `data.termination`، والخادم لا يبني الورقة
 * أصلاً بدونها.
 */
function finalSettlement(data) {
  const settlement = data.settlement ?? {};
  const totals = settlement.totals ?? {};
  const currency = settlement.currency ?? currencyOf(data);
  const slip = settlement.slip ?? null;
  const service = settlement.service ?? null;
  const blank = "................................";
  const money = (value) => formatMoney(value ?? 0, currency);

  const advancesList = settlement.advances ?? [];
  const custodyList = settlement.custody ?? [];
  const bonusesList = settlement.bonuses ?? [];

  const lastWorkingDay = settlement.lastWorkingDay ?? data.today;

  const nodes = [
    pairs([
      // تواريخ الورقة الثلاثة بصيغة واحدة كي تُقرأ المدة بلا لبس
      ["تاريخ المخالصة", formatDate(data.today)],
      ["آخر يوم عمل", formatDate(lastWorkingDay)],
      ["تاريخ المباشرة", data.employee?.hiredAt ? formatDate(data.employee.hiredAt) : "—"],
      ["مدة الخدمة", service ? `${service.text} (${service.totalDays} يوماً)` : "—"],
      ["جهة طلب إنهاء العقد", data.termination?.label],
      ...employeeRows(data),
    ]),
    section("سبب انتهاء العلاقة العمالية", [
      data.termination?.statement ??
        "انتهت العلاقة العمالية بين الطرفين اعتباراً من التاريخ الموضح أعلاه.",
    ]),
    table(
      ["البند", "المبلغ"],
      [
        [
          slip
            ? `صافي راتب شهر ${settlement.slipMonth ?? "—"}`
            : "صافي الراتب المستحق عن آخر شهر عمل",
          slip ? money(totals.slipNet) : blank,
        ],
        [
          bonusesList.length > 0
            ? `مكافآت معتمدة لم تُدرج في مسير (${bonusesList.length})`
            : "مكافآت معتمدة لم تُدرج في مسير",
          bonusesList.length > 0 ? money(totals.bonuses) : money(0),
        ],
        ["مكافأة نهاية الخدمة (تُحسب وفق النظام واللائحة)", blank],
        ["بدل الإجازة السنوية غير المستحقة", blank],
        ["مستحقات أخرى (تذكرة سفر، بدل إشعار، …)", blank],
        {
          className: "is-total",
          cells: ["إجمالي المستحقات المسجَّلة في النظام", money(totals.dues)],
        },
      ],
    ),
    table(
      ["الاستقطاع", "المبلغ"],
      [
        ["رصيد السلف غير المسدَّد", money(totals.advances)],
        ["قيمة العهد التي لم تُعَد", money(totals.custody)],
        ["استقطاعات أخرى (جزاءات، تلفيات، …)", blank],
        {
          className: "is-total",
          cells: ["إجمالي الاستقطاعات المسجَّلة في النظام", money(totals.deductions)],
        },
      ],
    ),
    pairs([
      ["صافي المخالصة من البنود المسجَّلة", money(totals.net)],
      ["يُضاف/يُخصم ما عُبِّئ يدوياً أعلاه", blank],
      ["صافي المبلغ المستلم فعلاً", blank],
      ["طريقة الصرف", "نقداً / تحويل بنكي / شيك"],
    ]),
  ];

  if (advancesList.length > 0) {
    nodes.push(
      section("تفصيل السلف القائمة", []),
      table(
        ["تاريخ الطلب", "السبب", "مبلغ السلفة", "شهر أول خصم", "المتبقي"],
  ...
  if (custodyList.length > 0) {
    nodes.push(
      section("تفصيل العهد التي لم تُعَد", []),
      table(
        ["الوصف", "النوع", "الرقم التسلسلي", "الكمية", "القيمة", "الحالة"],
        custodyList.map((row) => [
          row.itemName,
          CUSTODY_TYPES[row.itemType] ?? label(row.itemType ?? "other"),
          row.serialNumber || "—",
          row.quantity ?? 1,
          money(row.value),
          label(row.status ?? "issued"),
        ]),
      ),
    );
  }
  ...
  nodes.push(
    clauses([
      [
        "إقرار استلام المستحقات",
        `أقر أنا ${data.employee?.fullName ?? ""}، رقم الهوية/الإقامة ${
          data.employee?.nationalId ?? "—"
        }، باستلامي من ${
          data.company?.companyName ?? "المنشأة"
        } جميع مستحقاتي المالية عن كامل مدة عملي المنتهية في ${formatDate(lastWorkingDay)}، وتشمل الراتب والبدلات والأوفرتايم والمكافآت ومقابل الإجازات ومكافأة نهاية الخدمة وكل ما يترتب لي نظاماً حتى هذا التاريخ.`,
      ],
      [
        "الإبراء النهائي",
        "وأقر بأنه لم يبقَ لي عند المنشأة أي مبلغ أو حق أو مطالبة مالية أو عمالية من أي نوع، وأُبرئها ومالكها ومديريها إبراءً نهائياً عن كامل مدة العمل، وأتنازل عن أي دعوى أو مطالبة سابقة أو لاحقة تخص هذه المدة.",
      ],
      [
        "الاستقطاعات",
        "وأقر بصحة الاستقطاعات الموضحة أعلاه وبموافقتي على خصمها من مستحقاتي، وتشمل رصيد السلف غير المسدَّد وقيمة ما لم أُعِده من عهد.",
      ],
      [
        "العهد والمستندات",
        "وأقر بأني أعدت كل ما في حوزتي من عهد المنشأة وأجهزتها وزيّها ومفاتيحها ومستنداتها، وبأني لم أستبقِ أي نسخة من مستنداتها أو بياناتها.",
      ],
      [
        "استمرار السرية",
        "ويستمر التزامي بالحفاظ على سرية معلومات المنشأة ووصفاتها وأسعارها وبيانات عملائها ومورّديها بعد انتهاء العلاقة العمالية.",
      ],
      [
        "صحة البيانات",
        "وأقر بصحة بيانات الحساب البنكي/طريقة الصرف التي استلمت بها المبلغ، وأتحمّل مسؤولية أي خطأ في بياناتها.",
      ],
    ]),
    note(
      "البنود ذات الخانات الفارغة (مكافأة نهاية الخدمة، بدل الإجازة، المستحقات " +
        "والاستقطاعات الأخرى) لا يحسبها النظام، فتُعبّأ يدوياً وتُراجع مع القسم " +
        "المالي قبل التوقيع، ثم يُكتب صافي المبلغ المستلم فعلاً.",
    ),
    slip
      ? null
      : note(
          "لا يوجد مسير راتب محفوظ لآخر شهر عمل — يُكتب صافي الراتب المستحق يدوياً قبل التوقيع.",
        ),
  );
...
export const TEMPLATES = {
  ...
  final_settlement: {
    signatures: ["الموظف (المُقِر بالاستلام)", "الموارد البشرية", "القسم المالي"],
    render: finalSettlement,
  },
  leave: {
